...
public static class ProcessStateMetadataUtilities
{
    ...
    {
        if (!(Path.GetDirectoryName(fullFilepath) is { } directory && Directory.Exists(directory)))
        {
            throw new KernelException($"Directory for path '{fullFilepath}' does not exist, could not save process {processStateInfo.Name}");
        }

        if (!(Path.GetExtension(fullFilepath) is { } extension && !string.IsNullOrEmpty(extension) && extension == ".json"))
        {
